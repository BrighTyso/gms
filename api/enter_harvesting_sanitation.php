...
$field_cleanliness=$_GET['field_cleanliness'];
$diseased_leaves=$_GET['diseased_leaves'];
$pest_infestation=$_GET['pest_infestation'];
$tools_sanitation=$_GET['tools_sanitation'];

// userid, growerid, latitude, longitude, created_at, datetimes,seasonid, field_cleanliness, diseased_leaves, pest_infestation,tools_sanitation
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO harvesting_sanitation(userid, growerid, latitude, longitude, created_at, datetimes,seasonid, field_cleanliness, diseased_leaves, pest_infestation,tools_sanitation) VALUES ($userid, 
$growerid, '$latitude','$longitude','$created_at', '$datetimes',$seasonid, '$field_cleanliness', '$diseased_leaves', '$pest_infestation','$tools_sanitation')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','harvesting sanitation','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
