...
$midrib_start_date      = $_GET['midrib_start_date'];
$midrib_end_date        = $_GET['midrib_end_date'];
$midrib_temperature     = $_GET['midrib_temperature'];
$midrib_fully_dry       = $_GET['midrib_fully_dry'];
$snaps_cleanly          = $_GET['snaps_cleanly'];
$smoke_contamination    = $_GET['smoke_contamination'];
$midrib_mold_observed   = $_GET['midrib_mold_observed'];
$midrib_status          = $_GET['midrib_status'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO midrib_stage (
    userid, 
    growerid, 
    latitude, 
    longitude, 
   ...
   if ($conn->query($insert_sql)===TRUE) {
    ...
    // $last_id = $conn->insert_id;
     ...
     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','midrib stage','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
