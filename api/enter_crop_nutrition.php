...
$basal_type=$_GET['basal_type'];
$basal_app_rate=$_GET['basal_app_rate'];
$basal_app_date=$_GET['basal_app_date'];
$topping_type=$_GET['topping_type'];
$topping_app_rate=$_GET['topping_app_rate'];
$topping_app_date=$_GET['topping_app_date'];
$crop_symptoms=$_GET['crop_symptoms'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO crop_nutrition(userid,  growerid,  latitude,  longitude,  created_at,  seasonid,  basal_type, 
basal_app_rate, basal_app_date, topping_type,
topping_app_rate, topping_app_date, datetimes, crop_symptoms) VALUES ($userid,  $growerid, '$latitude',  '$longitude',  '$created_at',  $seasonid,  '$basal_type', 
'$basal_app_rate', '$basal_app_date', '$topping_type',
'$topping_app_rate', '$topping_app_date', '$datetimes', '$crop_symptoms')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','Crop Nutrition','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
