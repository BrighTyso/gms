...
if (isset($data->userid) &&  isset($data->sub_accountid) && isset($data->description)  && isset($data->currencyid) && isset($data->payment_typeid) && isset($data->amount)  && isset($data->receipt_num) && isset($data->transaction_at) && isset($data->created_at) && isset($data->seasonid)){
  ...
  //$transaction_at=date("Y-m-d",$trans_date);
...
//  }
 ...
 if ($found==0){
  ...
  $grower_farm_sql = "INSERT INTO revenue(userid,seasonid,currencyid,sub_accountid,payment_typeid,amount,receipt_num,description,transaction_at,created_at) VALUES ($userid,$seasonid,$currencyid,$sub_accountid,$payment_typeid,'$amount','$receipt_num','$description','$transaction_at','$created_at')";
     //$sql = "select * from login";
     if ($conn->query($grower_farm_sql)===TRUE) {
     
       $last_id = $conn->insert_id;       

       $debit_sql = "INSERT INTO revenue_debit(userid,revenueid,sub_accountid,created_at) VALUES ($userid,$last_id,$payment_typeid,'$created_at')";
     //$sql = "select * from login";
     if ($conn->query($debit_sql)===TRUE) {
     

            $credit_sql = "INSERT INTO revenue_credit(userid,revenueid,sub_accountid,created_at) VALUES ($userid,$last_id,$sub_accountid,'$created_at')";
         //$sql = "select * from login";
