...
$grade_separation=$_GET['grade_separation'];
$leaf_position_separation=$_GET['leaf_position_separation'];
$color_uniformity=$_GET['color_uniformity'];
$texture_consistency=$_GET['texture_consistency'];
$ripeness_accuracy=$_GET['ripeness_accuracy'];
$foreign_matter=$_GET['foreign_matter'];
$excessive_midribs=$_GET['excessive_midribs'];
$overall_quality=$_GET['overall_quality'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO leaf_grading_quality(userid,
growerid,
latitude,
longitude,
created_at,
...
excessive_midribs,
overall_quality,
datetimes) VALUES ($userid, $growerid, '$latitude', '$longitude', '$created_at', $seasonid, '$grade_separation', '$leaf_position_separation', '$color_uniformity', '$texture_consistency', '$ripeness_accuracy', '$foreign_matter', '$excessive_midribs', '$overall_quality', '$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','leaf grading quality','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
