...
$humidity_retain=$_GET['humidity_retain'];
$moisture_release=$_GET['moisture_release'];
$roof_leaks=$_GET['roof_leaks'];
$dripping=$_GET['dripping'];
$drainange=$_GET['drainange'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO barn_humidity(userid, 
growerid, 
latitude, 
longitude, 
created_at, 
...
'$drainange')";
   ...
   if ($conn->query($insert_sql)===TRUE) {
    ...
    // $last_id = $conn->insert_id;
     ...
     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','barn humidity','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
