...
if ($description=="All" || $description=="all") {
  $sql = "Select userid,latitude,longitude,sod.seasonid,time,eod,sod.created_at,eod_created_at,username from sod join users on users.id=sod.userid where sod.seasonid=$seasonid and  (sod.created_at between '$startDate' and '$endDate') order by created_at desc limit 20";
...
}else if($description!="All" || $description!="all" || $description!=""){
...
$sql = "Select userid,latitude,longitude,sod.seasonid,time,eod,sod.created_at,eod_created_at,username from sod join users on users.id=sod.userid where sod.seasonid=$seasonid and username='$description' and (sod.created_at between '$startDate' and '$endDate')  order by created_at desc limit 20" ;
$result = $conn->query($sql);
