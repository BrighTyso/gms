...
// $sql = "Select * from shipment_details where shipment='$shipment' or id=$shipment limit 1";
...
if ($storeid==0 || $productid==0) {
    ...
    $result = $conn->query($sql);
     ...
     $sql = "Select * from shipment_file where barcode='$barcode' limit 1";
    $result = $conn->query($sql);
     ...
     if ($result->num_rows > 0) {
       ...
       while($row = $result->fetch_assoc()) {
        // product id
         ...
         $shipment_detailsid=$row["shipment_detailsid"];
    ...
    $sql2 = "Select * from shipment_file where shipment_detailsid=$shipment_detailsid";
    $result2 = $conn->query($sql2);
    $shipment_count=$result2->num_rows;
...
if ($shipment_fileid==0) {
 ...
 //   if ($shipment_fileid>0 && $warehousing_sold_balesid>0) {
                 ...
                 $user_sql = "INSERT INTO arc_products(userid,storeitemid,old_quantity,new_quantity,created_at) VALUES ($userid,$last_id,$old_quantity,$new_quantity,'$created_at')";
                       ...
                       if ($conn->query($user_sql)===TRUE) {
                          ...
                          if ($conn->query($user_sql1)===TRUE) {
                            ...
                            $sql2 = "Select * from warehousing_storage_received_bales where shipment_detailsid=$shipment_detailsid";
                            $result2 = $conn->query($sql2);
                            $warehousing_storage_received_bales_count=$result2->num_rows;
                            ...
                            $bale_value=$mass*$price;
                             ...
                             if ($conn->query($user_sql1)===TRUE) {
                               ...
                               $temp=array("response"=>"success","barcode"=>$barcode,"warehousing_sold_balesid"=>$warehousing_sold_balesid,"received"=>$warehousing_storage_received_bales_count,"shipment_count"=>$shipment_count);
                                array_push($response,$temp);
