...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO harvesting_maturity(userid, 
growerid, latitude, longitude, created_at, datetimes,seasonid, MaturityStage, LeafColor, 
LeafTexture,LeafThickness,LeafElasticity,LeafGloss,MidribThickness,PercentageMature) VALUES ($userid, 
$growerid, '$latitude','$longitude','$created_at', '$datetimes',$seasonid, '$MaturityStage', '$LeafColor', 
'$LeafTexture','$LeafThickness','$LeafElasticity','$LeafGloss','$MidribThickness','$PercentageMature')";
   ...
   if ($conn->query($insert_sql)===TRUE) {
    ...
    // $last_id = $conn->insert_id;
     ...
     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','harvesting maturity','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
