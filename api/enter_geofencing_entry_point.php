...
$entry_time=validate($_GET['entry_time']);
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO grower_geofence_entry_point(userid,seasonid,growerid,entry_time,latitude,longitude,created_at,datetimes,description) VALUES ($userid,$seasonid,$growerid,'$entry_time','$latitude','$longitude','$created_at','$datetimes','')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','Geo Fencing Entry','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
