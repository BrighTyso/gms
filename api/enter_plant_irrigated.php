...
//http://192.168.1.190/gms/api/enter_plant_irrigated.php?userid=1&grower_num=777&latitude=13.2222&longitude=3.33376&created_at=23-09-2022&ha_planted_to_date=0&ha=1&date_of_plant=0&crop_stand_perc=0&crop_unifomity_perc=0&seasonid=1&sqliteid=1
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO plant_irrigated(userid,growerid,seasonid,latitude,longitude,ha_planted_to_date,ha,date_of_plant,crop_stand_perc,crop_unifomity_perc,created_at) VALUES ($userid,$growerid,$seasonid,'$lat','$long','$ha_planted_to_date','$ha','$date_of_plant','$crop_stand_perc','$crop_unifomity_perc','$created_at')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

    $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description) value($userid,$growerid,$seasonid,'$lat','$long','$created_at','Plant Irrigated');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
