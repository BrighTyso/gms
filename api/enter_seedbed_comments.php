...
$seedbed_comment1=validate($_GET['seedbed_comment1']);
$seedbed_comment2=validate($_GET['seedbed_comment2']);
$seedbed_comment3=validate($_GET['seedbed_comment3']);
$seedbed_comment4=validate($_GET['seedbed_comment4']);

$seedbed_comment5=validate($_GET['seedbed_comment5']);
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO seedbed_comments(userid,  seasonid,  growerid,  latitude,  longitude,  created_at,  seedbed_comment1,  
seedbed_comment2,  seedbed_comment3,  seedbed_comment4,  seedbed_comment5,datetimes) VALUES ($userid,$seasonid,$growerid,'$latitude',  '$longitude',  '$created_at','$seedbed_comment1',  '$seedbed_comment2','$seedbed_comment3','$seedbed_comment4','$seedbed_comment5','$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','seedbed comments','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
