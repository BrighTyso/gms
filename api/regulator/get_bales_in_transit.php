...
$sql = "Select distinct dispatch_note.id,note,horse_num,trailer_num,dispatch_note_total_dispatched.quantity as dispatch_quantity,dispatch_note_total_received.quantity as received_bales,name from dispatch_note join dispatch_note_total_dispatched on dispatch_note.id=dispatch_note_total_dispatched.dispatch_noteid join dispatch_note_total_received on dispatch_note.id=dispatch_note_total_received.dispatch_noteid join users on dispatch_note.userid=users.id where dispatch_note.seasonid=$seasonid and (dispatch_note.receiverid=$userid)";
...
$result = $conn->query($sql);
 ...
 if ($result->num_rows > 0) {
   ...
   while($row = $result->fetch_assoc()) {
    ...
    $temp=array("note"=>$row["note"],"horse_num"=>$row["horse_num"],"trailer_num"=>$row["trailer_num"],"dispatch_quantity"=>$row["dispatch_quantity"],"received_bales"=>$row["received_bales"],"name"=>$row["name"],"id"=>$row["id"]);
...
$sql = "Select distinct dispatch_note.id,note,horse_num,trailer_num,dispatch_note_total_dispatched.quantity as dispatch_quantity,dispatch_note_total_received.quantity as received_bales,name from dispatch_note join dispatch_note_total_dispatched on dispatch_note.id=dispatch_note_total_dispatched.dispatch_noteid join dispatch_note_total_received on dispatch_note.id=dispatch_note_total_received.dispatch_noteid join users on dispatch_note.userid=users.id where dispatch_note.seasonid=$seasonid  and (dispatch_note.receiverid=$userid) and (trailer_num='$description' or horse_num='$description' or note='$description' or users.name='$description' or users.surname='$description'  or users.username='$description' or dispatch_note.id='$description')  " ;
