...
$harvest_readiness=$_GET['harvest_readiness'];
 $harvest_date=$_GET['harvest_date'];
  $harvest_action=$_GET['harvest_action'];
  $delay_reason=$_GET['delay_reason'];
  $quality_expected=$_GET['quality_expected'];

// userid, growerid, latitude, longitude, created_at, datetimes,seasonid, harvest_readiness, harvest_date, harvest_action,delay_reason,quality_expected
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO harvesting_readiness(userid, growerid, latitude, longitude, created_at, datetimes,seasonid, harvest_readiness, harvest_date, harvest_action,delay_reason,quality_expected) VALUES ($userid, 
$growerid, '$latitude','$longitude','$created_at', '$datetimes',$seasonid, '$harvest_readiness', '$harvest_date', '$harvest_action','$delay_reason','$quality_expected')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','harvesting readiness','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
