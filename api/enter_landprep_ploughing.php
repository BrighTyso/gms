...
$PloughingOptions=validate($_GET['PloughingOptions']);
$dateOfPloughing=validate($_GET['dateOfPloughing']);
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO landprep_ploughing(userid,  growerid,  latitude,  longitude,  created_at,  seasonid,  PloughingOptions,  
dateOfPloughing,datetimes) VALUES ($userid,  $growerid,  '$latitude',  '$longitude',  '$created_at',  $seasonid,  '$PloughingOptions',  
'$dateOfPloughing','$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','landprep ploughing','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
