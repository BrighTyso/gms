...
if (!empty($_POST['image'])  && isset($_POST['userid']) && isset($_POST['seasonid'])  && isset($_POST['grower_num']) && isset($_POST['id'])){
		...
		//  }
		 ...
		 if ($image_found==0 && $growerid>0) {
		 	...
		 	$file_name=$_POST['grower_num'] ."-id-".time().".jpg";
			$path="../images/".$file_name;

			if (file_put_contents($path, base64_decode($_POST['image']))) {
				// code...
				$grower_farm_sql = "INSERT INTO grower_id_image(userid,seasonid,growerid,image_location,created_at) VALUES ($userid,$seasonid,$growerid,'$file_name','$created_at')";
