...
$sql = "Select * from grower_farm_image  where growerid=$growerid and seasonid=$seasonid";
 ...
 if ($result->num_rows > 0) {
   ...
   while($row = $result->fetch_assoc()) {
    // echo "id: " . $row["id"]. " - Name: " . $row["firstname"]. " " . $row["lastname"]. "<br>";
     ...
     $temp=array("latitude"=>$row["latitude"],"longitude"=>$row["longitude"],"id"=>$row["id"],"image_location"=>$row["image_location"]);
