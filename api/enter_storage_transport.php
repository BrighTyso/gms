...
$storage_dry=$_GET['storage_dry'];
$bale_density=$_GET['bale_density'];
$bales_raised=$_GET['bales_raised'];
$rodent_damag=$_GET['rodent_damage'];
$transport_ready=$_GET['transport_ready'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO storage_transport(userid,
growerid,
latitude,
longitude,
created_at,
...
rodent_damage,
transport_ready,
datetimes) VALUES ($userid, $growerid, '$latitude', '$longitude', '$created_at', $seasonid, '$storage_dry', '$bale_density', '$bales_raised', '$rodent_damag', '$transport_ready', '$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','storage transport','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
