...
if (isset($data->userid) && isset($data->storeitemid)  && isset($data->quantity) && isset($data->created_at)){
...
$sql = "Select * from store_hold_area where store_itemid=$storeitemid and quantity>0 and quantity>=$quantity limit 1";
$result = $conn->query($sql);
 ...
 $sql = "Select * from store_items where id=$storeitemid limit 1";
$result = $conn->query($sql);
...
if ($found>0) {
   ...
   $user_sql1 = "update store_items set quantity=quantity+$quantity  where id=$storeitemid ";
        ...
        $user_sql = "INSERT INTO arc_products(userid,storeitemid,old_quantity,new_quantity,created_at) VALUES ($userid,$storeitemid,$old_quantity,$new_quantity,'$created_at')";
           ...
           if ($conn->query($user_sql)===TRUE) {
              ...
              $arc_products_id = $conn->insert_id;
                ...
                $user_sql1 = "INSERT INTO arc_store_items(userid,storeid,productid,quantity,arc_productid,created_at,description,quantity_balance) VALUES ($userid,$storeid,$productid,$quantity,$arc_products_id,'$created_at','TRANSFER FROM HOLD AREA',$quantity)";
