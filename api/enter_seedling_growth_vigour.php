...
$seedling_health=validate($_GET['seedling_health']);
$seedling_health_rate=validate($_GET['seedling_health_rate']);
$seedling_growth=validate($_GET['seedling_growth']);
$seedling_stage_date=validate($_GET['seedling_stage_date']);
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO seedling_growth_vigour(userid, seasonid,  growerid,  latitude, longitude,created_at,seedling_health,seedling_health_rate,seedling_growth,seedling_stage_date,datetimes) VALUES ($userid,$seasonid,$growerid,'$latitude', '$longitude','$created_at','$seedling_health','$seedling_health_rate','$seedling_growth','$seedling_stage_date','$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','seedling growth vigour','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
