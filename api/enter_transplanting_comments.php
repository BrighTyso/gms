...
$transplant_comment1=validate($_GET['transplant_comment1']);
$transplant_comment2=validate($_GET['transplant_comment2']);
$transplant_comment3=validate($_GET['transplant_comment3']);
$transplant_comment4=validate($_GET['transplant_comment4']);
$transplant_comment5=validate($_GET['transplant_comment5']);
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO transplanting_comments(userid,growerid,latitude,  longitude,  created_at,seasonid,transplant_comment1,transplant_comment2,transplant_comment3,  transplant_comment4,transplant_comment5) VALUES ($userid,$growerid,'$latitude',  '$longitude','$created_at',$seasonid,'$transplant_comment1','$transplant_comment2',  '$transplant_comment3','$transplant_comment4','$transplant_comment5')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','transplanting comments','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
