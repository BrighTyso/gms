...
$exit_time=validate($_GET['exit_time']);
$created_at=validate($_GET['created_at']);
$minutes_with_grower=validate($_GET['minutes_with_grower']);
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO grower_geofence_exit_point(userid,seasonid,growerid,exit_time,latitude,longitude,created_at,datetimes,description,minutes_with_grower) VALUES ($userid,$seasonid,$growerid,'$exit_time','$latitude','$longitude','$created_at','$datetimes','',$minutes_with_grower)";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','Geo Fencing Exit','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
