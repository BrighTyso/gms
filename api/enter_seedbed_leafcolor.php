...
$bed_number=validate($_GET['bed_number']);
$seedbed_leaf_color=validate($_GET['seedbed_leaf_color']);
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO seedbed_leafcolor(userid,seasonid,growerid,latitude,longitude,created_at,bed_number,seedbed_leaf_color,datetimes) VALUES ($userid,$seasonid,$growerid,'$latitude','$longitude','$created_at','$bed_number','$seedbed_leaf_color','$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','seedbed leafcolor','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
