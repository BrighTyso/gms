...
$monitoring_frequency = $_GET['monitoring_frequency'];
$vent_strategy        = $_GET['vent_strategy'];
$heat_source          = $_GET['heat_source'];
$smoke_emissions      = $_GET['smoke_emissions'];
$env_accessibility    = $_GET['env_accessibility'];
$smoke_contamination  = $_GET['smoke_contamination'];
$env_overall_status   = $_GET['env_overall_status'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO curing_environment (
    userid, 
    growerid, 
    latitude, 
    longitude, 
   ...
   if ($conn->query($insert_sql)===TRUE) {
    ...
    // $last_id = $conn->insert_id;
     ...
     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','curing environment','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
