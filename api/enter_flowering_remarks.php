...
$harvesting_recommendations=$_GET['harvesting_recommendations'];
$nutrition_management=$_GET['nutrition_management'];
$sucker_control_remark=$_GET['sucker_control_remark'];
$pest_disease_treatment=$_GET['pest_disease_treatment'];
$field_sanitation=$_GET['field_sanitation'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO flowering_remarks(userid,  growerid,  latitude,  longitude,  created_at,  seasonid,  harvesting_recommendations,  
nutrition_management,  sucker_control_remark, pest_disease_treatment, field_sanitation, datetimes) VALUES ($userid,  $growerid,  '$latitude',  '$longitude',  '$created_at',  $seasonid,  '$harvesting_recommendations',  
'$nutrition_management',  '$sucker_control_remark', '$pest_disease_treatment', '$field_sanitation', '$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','flowering remarks','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
