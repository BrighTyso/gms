...
$plantedDate=validate($_GET['plantedDate']);
$intendedHa=validate($_GET['intendedHa']);
$bed_type=validate($_GET['bed_type']);
$seedVariety=validate($_GET['seedVariety']);

$germination_percentage=validate($_GET['germination_percentage']);
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO seed_germination(userid,seasonid,growerid,latitude,longitude,created_at,plantedDate,intendedHa,bed_type,seedVariety,germination_percentage,datetimes) VALUES ($userid,$seasonid,$growerid,'$latitude','$longitude','$created_at','$plantedDate','$intendedHa','$bed_type','$seedVariety','$germination_percentage','$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;
     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','seed germination','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
