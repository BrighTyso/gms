...
$yellowing_start_date  = $_GET['yellowing_start_date'];
$yellowing_end_date    = $_GET['yellowing_end_date'];
$yellowing_temperature = $_GET['yellowing_temperature'];
$relative_humidity     = $_GET['relative_humidity'];
$ventilation           = $_GET['ventilation'];
$uniformity_yellowing  = $_GET['uniformity_yellowing'];
$green_patches         = $_GET['green_patches'];
$leaf_pliability       = $_GET['leaf_pliability'];
$yellowing_status      = $_GET['yellowing_status'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO yellowing_stage (
    userid, 
    growerid, 
    latitude, 
    longitude, 
    ...
    seasonid, 
    barn_number, 
    yellowing_start_date, 
    yellowing_end_date, 
    yellowing_temperature, 
    relative_humidity, 
    ventilation, 
    uniformity_yellowing, 
    green_patches, 
    leaf_pliability, 
    yellowing_status, 
    datetimes
   ...
   if ($conn->query($insert_sql)===TRUE) {
    ...
    // $last_id = $conn->insert_id;
     ...
     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','yellowing stage','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
