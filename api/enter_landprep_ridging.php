...
$ridging_done=validate($_GET['ridging_done']);
$ridging_done_date=validate($_GET['ridging_done_date']);
$inter_rowspacing_done=validate($_GET['inter_rowspacing_done']);
$inter_rowspacing_measurement=validate($_GET['inter_rowspacing_measurement']);



$inrow_spacing_done=validate($_GET['inrow_spacing_done']);
$inrow_spacing_measurement=validate($_GET['inrow_spacing_measurement']);
$inrow_pockets_done=validate($_GET['inrow_pockets_done']);
$inrow_pockets_count=validate($_GET['inrow_pockets_count']);
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO landprep_ridging(userid,  growerid,  latitude,  longitude,  created_at,  seasonid,  ridging_done,  ridging_done_date,  
inter_rowspacing_done,  inter_rowspacing_measurement,  inrow_spacing_done,  
inrow_spacing_measurement,  inrow_pockets_done ,  inrow_pockets_count,datetimes) VALUES ($userid,  $growerid,  '$latitude',  '$longitude',  '$created_at',  $seasonid,  '$ridging_done',  '$ridging_done_date',  
'$inter_rowspacing_done',  '$inter_rowspacing_measurement',  '$inrow_spacing_done',  
'$inrow_spacing_measurement',  '$inrow_pockets_done' ,  '$inrow_pockets_count','$datetimes'  )";
