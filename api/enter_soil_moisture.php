...
$soil_structure=validate($_GET['soil_structure']);
$moisture_state=validate($_GET['moisture_state']);
$field_irrigation=validate($_GET['field_irrigation']);
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO soil_moisture(userid,growerid,latitude,longitude,  created_at,seasonid,soil_structure,moisture_state,datetimes,field_irrigation) VALUES ($userid,$growerid,'$latitude','$longitude','$created_at',$seasonid,'$soil_structure', '$moisture_state','$datetimes','$field_irrigation')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','Soil Moisture','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
