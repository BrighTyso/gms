...
$observed_disease=$_GET['observed_disease'];
$disease_effect_severity=$_GET['disease_effect_severity'];
$disease_treated_group=$_GET['disease_treated_group'];
$applied_fungicide=$_GET['applied_fungicide'];
$fungicide_app_date=$_GET['fungicide_app_date'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO disease_management(userid,  growerid,  latitude,  longitude,  created_at,  seasonid,  observed_disease, disease_effect_severity, disease_treated_group, applied_fungicide, datetimes, fungicide_app_date) VALUES ($userid,  $growerid,  '$latitude',  '$longitude',  '$created_at',  $seasonid,  '$observed_disease', '$disease_effect_severity', '$disease_treated_group', '$applied_fungicide', '$datetimes', '$fungicide_app_date')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','Disease Management','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
