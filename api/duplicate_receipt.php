...
for ($i=0; $i < $records ; $i++) { 
    // code...
$sqlx = "Select distinct * from loans join growers on growers.id=loans.growerid where (growers.grower_num='$description') and (loans.seasonid=$seasonid and receipt_number='$receipt_number') ";
...
if ($resultx->num_rows > 0) {
 ...
 while($rowx = $resultx->fetch_assoc()) {
  // echo "id: " . $row["id"]. " - Name: " . $row["firstname"]. " " . $row["lastname"]. "<br>";
    ...
    $growerid=$rowx['growerid'];
    $lat=$rowx['latitude'];
    $long=$rowx['longitude'];
    $quantity=$rowx['quantity'];
    $created_at=$data->created_at;
    
    $productid=$rowx['productid'];
    $seasonid=$rowx['seasonid'];
    $receipt_number=0;
