...
$sql = "Select * from fuel_allocation join users on users.id=fuel_allocation.fuel_userid  where  seasonid=$seasonid";
 ...
 if ($result->num_rows > 0) {
   ...
   while($row = $result->fetch_assoc()) {
    // echo "id: " . $row["id"]. " - Name: " . $row["firstname"]. " " . $row["lastname"]. "<br>";
     ...
     $temp=array("name"=>$row["name"],"surname"=>$row["surname"],"quantity"=>$row["quantity"],"allocation_date"=>$row["allocation_date"] ,"created_at"=>$row["created_at"]);
