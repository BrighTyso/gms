...
$vent_type=$_GET['vent_type'];
$vent_condition=$_GET['vent_condition'];
$vent_count=$_GET['vent_count'];
$vent_ease=$_GET['vent_ease'];
$airflow_obstruction=$_GET['airflow_obstruction'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO barn_ventilation(userid, 
growerid, latitude, longitude, created_at, datetimes,seasonid, barn_number, vent_type, vent_condition, vent_count,vent_ease,airflow_obstruction) VALUES ($userid, 
$growerid, 
'$latitude', 
'$longitude', 
'$created_at', 
'$datetimes',
$seasonid, '$barn_number', '$vent_type', '$vent_condition', '$vent_count','$vent_ease','$airflow_obstruction')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','barn ventilation','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
