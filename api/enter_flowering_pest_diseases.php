...
$crop_stage=$_GET['crop_stage'];
$pest_name=$_GET['pest_name'];
$pest_severity=$_GET['pest_severity'];

$pest_treated=$_GET['pest_treated'];
$pesticide_applied=$_GET['pesticide_applied'];
$pesticide_app_date=$_GET['pesticide_app_date'];
$disease_name=$_GET['disease_name'];
$disease_severity=$_GET['disease_severity'];
$disease_treated=$_GET['disease_treated'];
$sanitation_status=$_GET['sanitation_status'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO flowering_pest_diseases(userid,  growerid,  latitude,  longitude,  created_at,  seasonid,  crop_stage,  pest_name,  pest_severity, 
pest_treated, pesticide_applied, pesticide_app_date, disease_name, disease_severity, disease_treated, sanitation_status, datetimes) VALUES ($userid,  $growerid,  '$latitude',  '$longitude',  '$created_at',  $seasonid,  '$crop_stage',  '$pest_name',  '$pest_severity', 
'$pest_treated', '$pesticide_applied', '$pesticide_app_date', '$disease_name', '$disease_severity', '$disease_treated', '$sanitation_status', '$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','flowering pest diseases','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
