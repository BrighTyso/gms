  ...
  if ($growerid>0) {
   ...
   if ($conn->query($insert_sql)===TRUE) {
    ...
    // $last_id = $conn->insert_id;
     ...
     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description) value($userid,$growerid,$seasonid,'$lat','$long','$created_at','Plant Dry Land');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
