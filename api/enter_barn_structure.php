...
$grower_num=validate($_GET['grower_num']);
 ...
 $barn_number=$_GET['barn_number'];
  $barn_type=$_GET['barn_type']; 
$barn_roof=$_GET['barn_roof'];
 $barn_walls=$_GET['barn_walls'];
  $barn_doors=$_GET['barn_doors'];
  $barn_stability=$_GET['barn_stability'];
  $barn_floor=$_GET['barn_floor'];
  $barn_termite_rot=$_GET['barn_termite_rot'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO barn_structure(userid, growerid, latitude, longitude, datetimes,created_at, seasonid, barn_number, barn_type, 
barn_roof, barn_walls, barn_doors,barn_stability,barn_floor,barn_termite_rot) VALUES ($userid, 
$growerid, 
'$latitude', 
'$longitude', 
'$created_at', 
'$datetimes',
$seasonid, '$barn_number', '$barn_type', 
'$barn_roof', '$barn_walls', '$barn_doors','$barn_stability','$barn_floor','$barn_termite_rot')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','barn structure','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
