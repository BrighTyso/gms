...
$r = $conn->query("
    SELECT
        ...
        COALESCE((SELECT SUM(CAST(sh2.quantity AS DECIMAL(10,2))) FROM scheme_hectares sh2 WHERE sh2.schemeid=s.id),0) AS total_hectares,
        ...
        (SELECT COUNT(*)
         FROM loans l2
         LEFT JOIN ($priceSubquery) pr2 ON pr2.productid=l2.productid AND pr2.splitid=l2.splitid AND pr2.seasonid=l2.seasonid
         WHERE l2.seasonid=$seasonId
           AND l2.growerid IN (SELECT shg2.growerid FROM scheme_hectares_growers shg2 JOIN scheme_hectares sh3 ON sh3.id=shg2.scheme_hectaresid WHERE sh3.schemeid=s.id)
           AND l2.growerid NOT IN (SELECT growerid FROM blocked_growers WHERE seasonid=$seasonId)) AS loan_count,
        -- Loan value
        (SELECT COALESCE(SUM(COALESCE(pr2.amount,0)*l2.quantity),0)
         ...
         WHERE l2.seasonid=$seasonId
           AND l2.growerid IN (SELECT shg2.growerid FROM scheme_hectares_growers shg2 JOIN scheme_hectares sh3 ON sh3.id=shg2.scheme_hectaresid WHERE sh3.schemeid=s.id)
           AND l2.growerid NOT IN (SELECT growerid FROM blocked_growers WHERE seasonid=$seasonId)) AS loan_value,
        -- Recovery
        (SELECT COALESCE(SUM(lp2.amount),0)
         FROM loan_payments lp2
         WHERE lp2.seasonid=$seasonId
           AND lp2.growerid IN (SELECT shg2.growerid FROM scheme_hectares_growers shg2 JOIN scheme_hectares sh3 ON sh3.id=shg2.scheme_hectaresid WHERE sh3.schemeid=s.id)
           AND lp2.growerid NOT IN (SELECT growerid FROM blocked_growers WHERE seasonid=$seasonId)) AS recovered
    FROM scheme s
...
.kpi-val{font-family:'Syne',sans-serif;font-size:22px;font-weight:900;line-height:1;margin-bottom:4px;
  white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.kpi-label{font-size:9px;text-transform:uppercase;letter-spacing:.5px;color:var(--muted);white-space:nowrap}
.kpi-sub{font-size:10px;color:var(--muted);margin-top:6px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
    ...
    <div class="card">
      ...
      <div class="aging-grid">
        <div class="aging-cell">
          <div class="aging-val" style="color:var(--green)">$<?=number_format($agingBuckets['current']??0,0)?></div>
          <div class="aging-lbl">0–30 days</div>
        </div>
        <div class="aging-cell">
          <div class="aging-val" style="color:var(--amber)">$<?=number_format($agingBuckets['30']??0,0)?></div>
          <div class="aging-lbl">31–60 days</div>
        </div>
        <div class="aging-cell">
          <div class="aging-val" style="color:var(--red)">$<?=number_format($agingBuckets['60']??0,0)?></div>
          <div class="aging-lbl">61–90 days</div>
        </div>
        <div class="aging-cell">
          <div class="aging-val" style="color:#ff2020">$<?=number_format($agingBuckets['90plus']??0,0)?></div>
    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:8px 16px">
        ...
        <div class="officer-row">
          ...
          <div class="officer-val">$<?=number_format($ro['paid'],0)?></div>
        </div>
      ...
      <div class="card-body">
        ...
        <div class="bar-row">
          <div class="bar-label">
            ...
            <span class="val"><?=$tp['payment_count']?> pmts · $<?=number_format($tp['total_paid'],2)?></span>
          </div>
  ...
  <div class="card" style="margin-bottom:24px">
    ...
    <table class="data-table">
      ...
      <tr>
        ...
        <td style="text-align:right;color:var(--green);font-weight:700">$<?=number_format($p['amount'],2)?></td>
  ...
  <div style="display:grid;gap:16px;margin-bottom:24px">
    ...
    <div class="card" style="<?=$isActive?'border-color:var(--green)':''?>">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(130px,1fr));gap:1px;background:var(--border)">
        ...
        <div style="background:var(--surface);padding:12px 14px">
          <div style="font-family:'Syne',sans-serif;font-size:20px;font-weight:800;color:var(--green)">$<?=number_format($sc['loan_value'],2)?></div>
          <div style="font-size:8px;text-transform:uppercase;color:var(--muted);margin-top:2px">Loan Value</div>
        </div>
        <div style="background:var(--surface);padding:12px 14px">
          <div style="font-family:'Syne',sans-serif;font-size:20px;font-weight:800;color:var(--blue)">$<?=number_format($sc['recovered'],2)?></div>
  ...
  <div class="card" style="margin-bottom:24px">
    ...
    <div class="card-body" style="padding:8px 16px">
      ...
      <div class="officer-row">
        ...
        <div class="officer-val" style="color:var(--purple)">$<?=number_format($ro['total_rv'],2)?></div>
      </div>
      <?php endforeach?>
    </div>
  </div>
    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:8px 16px">
        ...
        <div class="officer-row">
          ...
          <div class="officer-val" style="color:var(--blue)">$<?=number_format($wo['total_wc'],2)?></div>
        </div>
        <?php endforeach; endif?>
      </div>
    </div>
      ...
      <div class="card-body">
        ...
        <div class="bar-row">
          <div class="bar-label">
            ...
            <span class="val"><?=$wg['wc_count']?> entries · $<?=number_format($wg['total_wc'],2)?></span>
          </div>
    ...
    <div class="card">
      ...
      <table class="data-table">
        ...
        <tr>
          ...
          <td style="text-align:right;color:var(--blue);font-weight:700">$<?=number_format($wc['amount'],2)?></td>
        </tr>
        <?php endforeach?>
        </tbody>
      </table>
      ...
      <div class="card-body">
        ...
        <div class="bar-row">
          <div class="bar-label">
            ...
            <span class="val"><?=number_format($p['qty'])?> <?=htmlspecialchars($p['units'])?> · $<?=number_format($p['unit_price'],2)?>/unit · $<?=number_format($p['total_cost'],2)?></span>
          </div>
    ...
    <div class="card">
      ...
      <div class="card-body" style="padding:8px 16px">
        ...
        <div class="officer-row">
          ...
          <div class="officer-val">$<?=number_format($o['total_value'],0)?></div>
        </div>
        <?php endforeach; endif?>
      </div>
    </div>
    ...
    <div class="card">
      <div class="card-head" style="background:var(--surface2)">
        ...
        <div style="display:flex;gap:12px;font-size:10px">
          <span style="color:var(--green)">$<?=number_format($oData['total_value'],2)?></span>
      ...
      <table class="data-table">
        ...
        <tr>
          ...
          <td style="color:var(--muted)">$<?=number_format($ol['unit_price'],2)?></td>
          <td style="color:var(--green)">$<?=number_format($ol['total'],2)?></td>
        </tr>
        <?php endforeach?>
        </tbody>
      </table>
    ...
    <div class="card-body">
      ...
      <div class="bar-row">
        <div class="bar-label">
          ...
          <span class="val"><?=$g['loan_count']?> loans · $<?=number_format($g['total_value'],2)?></span>
        </div>
        <div class="bar-track"><div class="bar-fill" style="width:<?=$pct?>%;background:<?=$col?>"></div></div>
      </div>
      <?php endforeach; endif?>
    ...
    <div class="card">
      ...
      <div class="price-grid">
        ...
        <div class="price-chip">
          ...
          <div class="pc-amount">$<?=number_format($pr['amount'],2)?></div>
      ...
      <div style="overflow-x:auto">
        <table class="data-table">
          ...
          <tr>
            ...
            <td style="color:var(--muted)">$<?=number_format($u['unit_price'],2)?></td>
            <td style="color:var(--green)">$<?=number_format($u['computed_value'],2)?></td>
