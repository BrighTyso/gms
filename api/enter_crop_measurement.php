...
$crop_age=$_GET['crop_age'];
$avg_plant_height=$_GET['avg_plant_height'];
$avg_crop_vigor=$_GET['avg_crop_vigor']; 
$avg_crop_density=$_GET['avg_crop_density'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO crop_measurement(userid,growerid,  latitude,  longitude,  created_at,  seasonid,  crop_age, avg_plant_height, avg_crop_vigor, 
datetimes,avg_crop_density) VALUES ($userid,$growerid,  '$latitude',  '$longitude',  '$created_at',  $seasonid,  '$crop_age', '$avg_plant_height', '$avg_crop_vigor', 
'$datetimes','$avg_crop_density')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','Crop Measurement','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
