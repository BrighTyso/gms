...
$nitrogen_level=$_GET['nitrogen_level'];
$phosphorus_level=$_GET['phosphorus_level'];
$potassium_level=$_GET['potassium_level'];
$field_moisture=$_GET['field_moisture'];
$field_drought=$_GET['field_drought'];
$field_drought_severity=$_GET['field_drought_severity'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO nutrient_stress(userid,  growerid,   latitude,  longitude,  created_at,  seasonid,  crop_stage,  nitrogen_level,  phosphorus_level, potassium_level, field_moisture, field_drought, field_drought_severity, datetimes) VALUES ($userid,  $growerid,  '$latitude',  '$longitude',  '$created_at',  $seasonid,  '$crop_stage',  '$nitrogen_level',  '$phosphorus_level', '$potassium_level', '$field_moisture', '$field_drought', '$field_drought_severity', '$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','nutrient stress','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
