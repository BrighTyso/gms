...
$labour_availability=$_GET['labour_availability'];
$workers_experienced=$_GET['workers_experienced'];
$harvest_speed=$_GET['harvest_speed'];
$time_of_harvest=$_GET['time_of_harvest'];
$transport_readiness=$_GET['transport_readiness'];

// userid, growerid, latitude, longitude, created_at, datetimes,seasonid, labour_availability, workers_experienced, harvest_speed,time_of_harvest,transport_readiness
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO harvesting_labour(userid, growerid, latitude, longitude, created_at, datetimes,seasonid, labour_availability, workers_experienced, harvest_speed,time_of_harvest,transport_readiness) VALUES ($userid, 
$growerid, '$latitude','$longitude','$created_at', '$datetimes',$seasonid, '$labour_availability', '$workers_experienced', '$harvest_speed','$time_of_harvest','$transport_readiness')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','harvesting labour','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
