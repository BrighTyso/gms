...
$sql = "Select distinct grower_field_loans.id as loanid,products.id as productid,growers.name,growers.id,growers.surname,growers.grower_num,products.name as product_name,quantity,units,grower_field_loans.created_at, users.username,farmer_comment,adjustment_quantity,grower_field_loans.hectares  from grower_field_loans join growers on growers.id=grower_field_loans.growerid join products on grower_field_loans.productid=products.id  join users on users.id=grower_field_loans.userid  where grower_field_loans.seasonid=$seasonid and (grower_num='$description' or  users.username='$description' or province='$description' or  grower_num  like '%$description') order by grower_num limit 30";
$result = $conn->query($sql);
