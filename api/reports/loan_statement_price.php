...
$result1 = $conn->query($sql11);
 ...
 if ($result1->num_rows > 0) {
  ...
  $result2 = $conn->query($sql12);
   ...
   $sqlx = "select distinct file_manager.id,growerid,location_url,description,file_type,storages,file_manager.created_at,datetimes,grower_num,growers.name,surname,seasons.name as season_name from file_manager join growers on growers.id=file_manager.growerid join seasons on seasons.id=file_manager.seasonid where grower_num='$grower_num' and seasons.id=$seasonid order by seasons.id desc limit 1";
 ...
 if ($resultx->num_rows > 0) {
  ...
  if ($signature_url=="") {
    // code...
     ...
     $sqlx = "select distinct location_url from file_manager join growers on growers.id=file_manager.growerid  where grower_num='$grower_num' and file_manager.seasonid=$seasonid and description='Verification Signature' order by file_manager.id desc limit 1";
      $resultx = $conn->query($sqlx);
       ...
       if ($resultx->num_rows > 0) {
         ...
         while($rowx = $resultx->fetch_assoc()) {
          // echo "id: " . $row["id"]. " - Name: " . $row["firstname"]. " " . $row["lastname"]. "<br>";
            ...
            $signature_url=$rowx["location_url"];
...
$sql11 = "Select distinct growers.id,growers.name,growers.surname,growers.grower_num,growers.area,growers.id_num,receipt_number from  growers join active_growers on active_growers.growerid=growers.id join system_receipt_number on system_receipt_number.growerid=growers.id where active_growers.seasonid=$seasonid and (name ='$description' or grower_num='$description' or province='$description' or area='$description' or receipt_number='$description') limit 200";
...
$result1 = $conn->query($sql11);
 ...
 if ($result1->num_rows > 0) {
  ...
  $result2 = $conn->query($sql12);
   ...
   $sqlx = "select distinct file_manager.id,growerid,location_url,description,file_type,storages,file_manager.created_at,datetimes,grower_num,growers.name,surname,seasons.name as season_name from file_manager join growers on growers.id=file_manager.growerid join seasons on seasons.id=file_manager.seasonid where grower_num='$grower_num' and seasons.id=$seasonid and description='Identity' order by seasons.id desc limit 1";
$resultx = $conn->query($sqlx);
