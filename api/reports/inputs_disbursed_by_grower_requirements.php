...
if ($productid>0) {
  ...
  $sql = "Select distinct growers.phone,growers.id_num,area,products.id as productid,growers.name,growers.id,growers.surname,growers.grower_num,products.name as product_name,quantity,units,grower_field_loans.created_at, users.username  from grower_field_loans join growers on growers.id=grower_field_loans.growerid join products on grower_field_loans.productid=products.id join users on users.id=grower_field_loans.userid  where grower_field_loans.seasonid=$seasonid  and grower_field_loans.productid=$productid order by growers.grower_num ";
...
$sql111 = "Select distinct products.name as product_name  from grower_field_loans join growers on growers.id=grower_field_loans.growerid join products on grower_field_loans.productid=products.id join users on users.id=grower_field_loans.userid  where grower_field_loans.seasonid=$seasonid order by growers.grower_num,products.name ";
...
$sql111 = "Select distinct growers.phone,growers.id_num,area,growers.name,growers.id,growers.surname,growers.grower_num  from grower_field_loans join growers on growers.id=grower_field_loans.growerid join products on grower_field_loans.productid=products.id join users on users.id=grower_field_loans.userid  where grower_field_loans.seasonid=$seasonid order by growers.grower_num,products.name ";
$result111 = $conn->query($sql111);
if ($result111->num_rows > 0) {
   // output data of each row
   while($row111 = $result111->fetch_assoc()) {
    ...
    $fieldOfficer="";
    $inputs=array();
    $sql = "Select distinct growers.phone,growers.id_num,area,products.id as productid,growers.name,growers.id,growers.surname,growers.grower_num,products.name as product_name,quantity,units,grower_field_loans.created_at, users.username  from grower_field_loans join growers on growers.id=grower_field_loans.growerid join products on grower_field_loans.productid=products.id join users on users.id=grower_field_loans.userid  where grower_field_loans.seasonid=$seasonid and growerid=$growerid_found order by growers.grower_num ";
$result = $conn->query($sql);
 ...
 if ($result->num_rows > 0) {
   ...
   while($row = $result->fetch_assoc()) {
    // echo "id: " . $row["id"]. " - Name: " . $row["firstname"]. " " . $row["lastname"]. "<br>";
        ...
        $temp=array("productid"=>$row["productid"],"id"=>$row["id"],"product_name"=>$row["product_name"],"quantity"=>$row["quantity"],"units"=>$row["units"],"created_at"=>$row["created_at"],"username"=>$row["username"]);
