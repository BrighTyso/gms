...
$sql111 = "Select distinct products.name as product_name  from grower_field_loans join growers on growers.id=grower_field_loans.growerid join products on grower_field_loans.productid=products.id join users on users.id=grower_field_loans.userid  where grower_field_loans.seasonid=$seasonid order by products.name ";
    ...
    $result1 = $conn->query($sql1);
     ...
     if ($result1->num_rows > 0) {
       ...
       while($row1 = $result1->fetch_assoc()) {
        ...
        $sql111 = "Select distinct products.name as product_name,products.id  from grower_field_loans  join products on grower_field_loans.productid=products.id   where grower_field_loans.seasonid=$seasonid order by products.name ";
        $result111 = $conn->query($sql111);
        if ($result111->num_rows > 0) {
           // output data of each row
           while($row111 = $result111->fetch_assoc()) {
          ...
          $sql = "SELECT
            ...
            SUM(CASE WHEN grower_field_loans.adjust = 1 THEN adjustment_quantity ELSE quantity END) AS quantities,
        ...
        JOIN
            ...
            join products on  grower_field_loans.productid=products.id 

            where grower_field_loans.seasonid=$seasonid and grower_field_loans.productid=$productid and users.id=$fieldOfficerid
