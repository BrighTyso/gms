...
//$sql11 = "Select growers.id from  growers join active_growers on growers.id=active_growers.growerid where active_growers.seasonid=$seasonid";
 ...
 $sql = "Select id,start_date,end_date from salary_dates_and_months order by id desc limit 1";
...
$sql2345 = "Select * from users where (rightsid=7 or rightsid=8 or rightsid=9 or rightsid=1) and  active=1 limit 6";
  $result2345 = $conn->query($sql2345);
   ...
   if ($result2345->num_rows > 0) {
    ...
    $field_officer_username=$row2345["username"];
    $fieldOfficerid=$row2345["id"];
...
$sql112 = "Select distinct * from  grower_field_officer  where grower_field_officer.seasonid=$seasonid and grower_field_officer.field_officerid=$fieldOfficerid";
$result12 = $conn->query($sql112);
$no_of_growers=$result12->num_rows;
$hectares=0;



$sql11 = "Select distinct quantity,scheme_hectares_growers.growerid from  scheme_hectares_growers join active_growers on active_growers.growerid=scheme_hectares_growers.growerid join grower_field_officer on grower_field_officer.growerid=scheme_hectares_growers.growerid join scheme_hectares on scheme_hectares.id=scheme_hectares_growers.scheme_hectaresid where active_growers.seasonid=$seasonid and grower_field_officer.field_officerid=$fieldOfficerid";
 ...
 if ($result1->num_rows > 0) {
  ...
  $result2 = $conn->query($sql12);
   ...
   if ($min_code=="") {
    ...
    $sql13 = "Select * from loan_payments where growerid=$growerid and seasonid=$seasonid and (payment_date between '$start_date' and  '$end_date')";
    ...
    $result3 = $conn->query($sql13);
     ...
     if ($result3->num_rows > 0) {
       ...
       while($row3 = $result3->fetch_assoc()) {
        ...
        $loans=array("creditor"=>$row3["reference_num"],"description"=>$row3["description"],"created_at"=>$row3["created_at"],"amount"=>$row3["amount"],"mass"=>$row3["mass"],"bales"=>$row3["bales"]);
...
$temp=array("data"=>$data1,"field_officer"=>$field_officer_username);
...
$sql2345 = "Select * from users where (username='$description' or name='$description' or surname='$description') and active=1";
  $result2345 = $conn->query($sql2345);
...
array_push($field_officer_data,$temp);
 ...
 echo json_encode($field_officer_data);
