...
$sql = "select * from lat_long  where userid=$fieldOfficerid and lat_long.seasonid=$seasonid";
  $result = $conn->query($sql);
$home_totals=$result->num_rows;
 

 $sql = "select * from seedbed_location  where userid=$fieldOfficerid and seedbed_location.seasonid=$seasonid ";
  $result = $conn->query($sql);
$seedbed_totals=$result->num_rows;
 


$sql = "select * from barn_location  where userid=$fieldOfficerid and barn_location.seasonid=$seasonid";
  $result = $conn->query($sql);
...
$sql = "select * from grower_farm where userid=$fieldOfficerid and grower_farm.seasonid=$seasonid ";
  $result = $conn->query($sql);
$farm_totals=$result->num_rows;



 $sql = "select distinct growers.surname,growers.name,growers.grower_num,growers.id  from visits join growers on growers.id=visits.growerid where  visits.seasonid=$seasonid and visits.userid=$fieldOfficerid  order by visits.created_at desc ";
...
$sql = "select distinct growers.grower_num,visits.created_at  from visits join growers on growers.id=visits.growerid where  visits.seasonid=$seasonid and visits.growerid=$growerid  order by visits.created_at desc ";
$result = $conn->query($sql);
$number_of_visits=$result->num_rows;
