...
// join growers on growers.id=scheme_hectares_growers.growerid join products on scheme_hectares_products.productid=products.id join users on users.id=scheme_hectares_growers.userid join total_disbursement on total_disbursement.productid=products.id where scheme_hectares.seasonid=$seasonid and scheme_hectares_growers.growerid=$growerid and total_disbursement.disbursement_trucksid=$disbursement_truckid order by grower_num";
  ...
  $sql = "Select distinct products.id as productid,growers.name,growers.id,growers.surname,growers.grower_num,products.name as product_name,scheme_hectares_products.quantity,units,package_units,total_disbursement.created_at,scheme_hectares.quantity as hectares,amount,prices.splitid from scheme join scheme_hectares on scheme_hectares.schemeid=scheme.id join scheme_hectares_products on scheme_hectares_products.scheme_hectaresid=scheme_hectares.id join scheme_hectares_growers on scheme_hectares_growers.scheme_hectaresid=scheme_hectares.id 
join growers on growers.id=scheme_hectares_growers.growerid join products on scheme_hectares_products.productid=products.id join users on users.id=scheme_hectares_growers.userid join total_disbursement on total_disbursement.productid=products.id join prices on prices.productid=products.id where scheme_hectares.seasonid=$seasonid and prices.seasonid=$seasonid and prices.splitid=loans.splitid and scheme_hectares_growers.growerid=$growerid and total_disbursement.disbursement_trucksid=$disbursement_truckid and (scheme_hectares_products.productid) not in (Select loans.productid from loans where loans.productid=scheme_hectares_products.productid and loans.growerid=growers.id and loans.seasonid=$seasonid) order by grower_num desc";
$result = $conn->query($sql);
...
$grower_hectares=$row["hectares"];
     ...
     $temp=array("package_units"=>$row["package_units"],"productid"=>$row["productid"],"product_name"=>$row["product_name"],"quantity"=>$row["quantity"],"units"=>$row["units"],"created_at"=>$row["created_at"],"username"=>$username,"receipt_number"=>$receipt_number,"loanid"=>"0","farmer_comment"=>"","adjustment_quantity"=>"0","hectares"=>$row["hectares"],"adjust"=>"0","amount"=>$row["amount"]);
...
$sql = "Select distinct products.id as productid,growers.name,growers.id,growers.surname,growers.grower_num,products.name as product_name,scheme_hectares_products.quantity,units,package_units,total_disbursement.created_at,scheme_hectares.quantity as hectares,amount,prices.splitid from scheme join scheme_hectares on scheme_hectares.schemeid=scheme.id join scheme_hectares_products on scheme_hectares_products.scheme_hectaresid=scheme_hectares.id join scheme_hectares_growers on scheme_hectares_growers.scheme_hectaresid=scheme_hectares.id 
join growers on growers.id=scheme_hectares_growers.growerid join products on scheme_hectares_products.productid=products.id join users on users.id=scheme_hectares_growers.userid join total_disbursement on total_disbursement.productid=products.id join prices on prices.productid=products.id where scheme_hectares.seasonid=$seasonid and prices.seasonid=$seasonid and prices.splitid=loans.splitid and scheme_hectares_growers.growerid=$growerid and total_disbursement.disbursement_trucksid=$disbursement_truckid order by grower_num desc";
$result = $conn->query($sql);
