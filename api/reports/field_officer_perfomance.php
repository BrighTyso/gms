 ...
 if ($result->num_rows > 0) {
   ...
   while($row = $result->fetch_assoc()) {
    // echo "id: " . $row["id"]. " - Name: " . $row["firstname"]. " " . $row["lastname"]. "<br>";
        ...
        $sql1 = "Select distinct growerid from visits where  userid=$userid and seasonid=$seasonid";
      $result1 = $conn->query($sql1);
       
      $visited_growers=$result1->num_rows;


      $sql2 = "Select distinct * from distance where  userid=$userid and seasonid=$seasonid";
      ...
      $sql50 = "Select distinct sod.userid,users.username,sod.created_at from sod join users on users.id=sod.userid join visits on visits.created_at=sod.created_at where sod.seasonid=$seasonid and sod.userid=$userid order by sod.created_at desc";
      ...
      $sql2 = "Select distinct * from hours_worked where  userid=$userid and seasonid=$seasonid";
      $result2 = $conn->query($sql2);
      ...
      $sql2 = "Select distinct * from fuel_allocation where  fuel_userid=$userid and seasonid=$seasonid";
      $result2 = $conn->query($sql2);
