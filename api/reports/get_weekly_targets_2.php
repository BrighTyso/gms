...
$daily_percent=0;
 ...
 from  weekly_plan  where  weekly_plan.seasonid=$seasonid and  weekly_plan.created_at between '$start_date' and '$end_date' order by start_date asc";
$result = $conn->query($sql);
if ($result->num_rows > 0) {
   // output data of each row
   while($row = $result->fetch_assoc()) {

    $temp=array("start_date"=>$row["start_date"],"end_date"=>$row["end_date"]);
    array_push($weeks,$temp);

   }
 }


 $sql211 = "Select distinct * from  users where (rightsid=8 or rightsid=9) and active=1";
 ...
 from users join weekly_plan on weekly_plan.userid=users.id where active=1 and weekly_plan.seasonid=$seasonid and  weekly_plan.created_at between '$start_date' and '$end_date' and users.id=$user_fetched  order by username,weekly_plan.created_at desc";
$result = $conn->query($sql);
 ...
 if ($result->num_rows > 0) {
   ...
   while($row = $result->fetch_assoc()) {
    // echo "id: " . $row["id"]. " - Name: " . $row["firstname"]. " " . $row["lastname"]. "<br>";
      ...
      if ($user_daily_total>0) {
        $avarage_percent=$daily_percent/6;
      }else{
        $avarage_percent=0;
      }
  ...
  $temp=array("username"=>$row211["username"],"data"=>$data1,"weeks"=>$weeks);
