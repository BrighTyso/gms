 ...
 if ($result->num_rows > 0) {
   ...
   while($row = $result->fetch_assoc()) {
    // echo "id: " . $row["id"]. " - Name: " . $row["firstname"]. " " . $row["lastname"]. "<br>";
      ...
      $sql2 = "Select distinct growerid,grower_num,name,visits.created_at,visits.description from  visits join growers on growers.id=visits.growerid where visits.userid=$userid and visits.seasonid=$seasonid and (visits.created_at between '$start' and '$end')";
      $result2 = $conn->query($sql2);
...
,"distance"=>$kms,"hours"=>$hours_worked,"visits"=>$visited_growers,"total_growers"=>$total_growers,"total_visits"=>$grower_visits,"working_days"=>$working_days,"fuel_allocation"=>$fuel_allocation,"start_of_days"=>$total_start_of_days,"vehicle_type"=>"motor bike","vehicle_type"=>"motor bike","terrain_type"=>"Mixed/Dirt","growers"=>$growers_data);
