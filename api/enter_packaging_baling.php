...
$packaging_type=$_GET['packaging_type'];
$bale_density=$_GET['bale_density'];
$leaf_alignment=$_GET['leaf_alignment'];
$wrapping_intact=$_GET['wrapping_intact'];
$bale_contamination=$_GET['bale_contamination'];
$bale_labeling=$_GET['bale_labeling'];
...
// then insert loan
  ...
  $insert_sql = "INSERT INTO packaging_baling(userid,
growerid,
latitude,
longitude,
created_at,
...
bale_contamination,
bale_labeling,
datetimes) VALUES ($userid, $growerid, '$latitude', '$longitude', '$created_at', $seasonid, '$packaging_type', '$bale_density', '$leaf_alignment', '$wrapping_intact', '$bale_contamination', '$bale_labeling', '$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','packaging baling','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
