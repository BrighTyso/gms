...
$firebreak_cleared=$_GET['firebreak_cleared'];
$chimney_safety=$_GET['chimney_safety'];
$heat_source_outside=$_GET['heat_source_outside'];
$fire_fighting=$_GET['fire_fighting'];
$electric_wiring_safe=$_GET['electric_wiring_safe'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO barn_fire_safety(userid, 
growerid, 
latitude, 
longitude, 
created_at, 
...
heat_source_outside,
fire_fighting,
electric_wiring_safe) VALUES ($userid, 
$growerid, 
'$latitude', 
'$longitude', 
'$created_at', 
...
'$heat_source_outside',
'$fire_fighting',
'$electric_wiring_safe')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','barn fire safety','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
