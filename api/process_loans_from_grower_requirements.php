...
// $sql123 = "Select distinct scheme_hectares_products.quantity as products_quantity,description,scheme_hectares.quantity as hectares,products.name,grower_num,scheme_hectares_products.productid from scheme join scheme_hectares on scheme_hectares.schemeid=scheme.id join scheme_hectares_products on scheme_hectares_products.scheme_hectaresid=scheme_hectares.id join products on products.id=scheme_hectares_products.productid join scheme_hectares_growers on scheme_hectares_growers.scheme_hectaresid=scheme_hectares.id join growers on growers.id=scheme_hectares_growers.growerid where (scheme_hectares_products.productid,scheme_hectares_products.quantity) not in (Select loans.productid,loans.quantity from loans where loans.productid=scheme_hectares_products.productid and loans.growerid=growers.id and loans.seasonid=$seasonid) and scheme_hectares.seasonid=$seasonid and scheme_hectares_products.active=1 and grower_num='$grower_num' order by scheme.description limit 100";
   ...
   if ($grower_num=="") {
      ...
      $sql12345 = "Select distinct grower_field_loans.id as loanid, products.id as productid,growers.id,growers.grower_num,products.name,grower_field_loans.quantity as products_quantity,units,grower_field_loans.created_at, users.username,farmer_comment,adjustment_quantity,grower_field_loans.hectares,adjust from grower_field_loans join growers on growers.id=grower_field_loans.growerid join products on grower_field_loans.productid=products.id join users on users.id=grower_field_loans.userid  where grower_field_loans.seasonid=$seasonid and (grower_field_loans.productid) not in (Select loans.productid from loans where loans.productid=grower_field_loans.productid and loans.growerid=growers.id and loans.seasonid=$seasonid) order by grower_num limit 1500";

   }else{

      $sql12345 = "Select distinct grower_field_loans.id as loanid, products.id as productid,growers.id,growers.grower_num,products.name,grower_field_loans.quantity as products_quantity,units,grower_field_loans.created_at, users.username,farmer_comment,adjustment_quantity,grower_field_loans.hectares,adjust from grower_field_loans join growers on growers.id=grower_field_loans.growerid join products on grower_field_loans.productid=products.id join users on users.id=grower_field_loans.userid  where grower_field_loans.seasonid=$seasonid and (grower_field_loans.productid) not in (Select loans.productid from loans where loans.productid=grower_field_loans.productid and loans.growerid=growers.id and loans.seasonid=$seasonid) and grower_num='$grower_num' order by grower_num limit 1500";

   }


  $result123 = $conn->query($sql12345);
...
$growerid=0;
$lat="Desktop Requirements";
$long="Desktop Requirements";


if ($row123["adjust"]==0) {
   $quantity=$row123["products_quantity"];
}else{
   $quantity=$row123["adjustment_quantity"];
}
...
if (isset($userid) && isset($description)  && isset($lat)  && isset($long)  && isset($productid) && isset($quantity) && isset($seasonid) && isset($created_at) && isset($hectares) && isset($trucknumber) && $already_processed==0){
...
if ($deduction_point==0) {
                      ...
                      $sql = "Select * from user_to_store where loan_userid=$userid and active=1 ";
                          ...
                          $result = $conn->query($sql1);
                           ...
                           $sql2 = "Select store.id,quantity,store_items.id as storeid from store join store_items on store.id=store_items.storeid where store.id=$storeid and productid=$productid and quantity>0 and quantity>=$quantity limit 1";
                             ...
                             $sql = "Select scheme_hectares_products.quantity from scheme join scheme_hectares on scheme_hectares.schemeid=scheme.id join scheme_hectares_products on scheme_hectares_products.scheme_hectaresid=scheme_hectares.id join scheme_hectares_growers on scheme_hectares_growers.scheme_hectaresid=scheme_hectares.id where scheme_hectares.seasonid=$seasonid and scheme_hectares_products.productid=$productid and scheme_hectares_products.active=1 and scheme_hectares_growers.growerid=$growerid";
                              $result = $conn->query($sql);
                               ...
                               $quantity_to_be_captured=$scheme_captured_quantity-$product_captured_quantity;
                                ...
                                // echo " to be capture ".$quantity_to_be_captured."\n";
                                // echo " product_captured_quantity ".$product_captured_quantity."\n";
                                // echo " scheme_captured_quantity ".$scheme_captured_quantity."\n";
                                // echo " quantity ".$quantity."\n";
                                // if ($quantity_to_be_captured>=$quantity) {
  ...
  if (($productid>0  && $growerid>0 && $verifyLoan==0)   &&  ($quantity_Enough>0) && $quantity_to_be_captured>=$quantity) {
...
//echo "storeid";
         ...
         //echo "quantity_Enough";

        }else if($quantity_to_be_captured<$quantity){

//echo "quantity_to_be_captured";
