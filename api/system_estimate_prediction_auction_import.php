...
$auctionid=0;
  ...
  $sql = "Select userid from auction_growers  where  growerid=$growerid and seasonid=$seasonid";
  $result = $conn->query($sql);
 ...
 if ($response==1 && $farm_response==0 && $grower_bales==0 && $auctionid>0){

	$grower_farm_sql = "INSERT INTO system_estimate_prediction(userid,seasonid,growerid,estimate,created_at) VALUES ($auctionid,$seasonid,$growerid,'$estimate','$created_at')";
	   //$sql = "select * from login";
	   if ($conn->query($grower_farm_sql)===TRUE) {
	   

	     $grower_bales = "INSERT INTO grower_number_of_bales(userid,seasonid,growerid,bales,created_at) VALUES ($auctionid,$seasonid,$growerid,$bales,'$created_at')";
     ...
     if ($conn->query($grower_bales1)===TRUE) {
      ...
      $insert_sql = "INSERT INTO tags_total(userid,grower_number_of_balesid,tags_total) VALUES ($auctionid,$last_id,$bales)";
