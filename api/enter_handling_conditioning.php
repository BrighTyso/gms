...
$over_handled=$_GET['over_handled'];
$leaf_breakage=$_GET['leaf_breakage'];
$re_drying=$_GET['re_drying'];
$proper_conditioning=$_GET['proper_conditioning'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO handling_conditioning(userid,
growerid,
latitude,
longitude,
created_at,
...
re_drying,
proper_conditioning,
datetimes) VALUES ($userid, $growerid, '$latitude', '$longitude', '$created_at', $seasonid, '$over_handled', '$leaf_breakage', '$re_drying', '$proper_conditioning', '$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','handling conditioning','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
