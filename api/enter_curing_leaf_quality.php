...
$leaf_colour         = $_GET['leaf_colour'];
$curing_leaf_texture = $_GET['curing_leaf_texture'];
$leaf_uniformity     = $_GET['leaf_uniformity'];
$mould_presence      = $_GET['mould_presence'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO curing_leaf_quality (
    userid, 
    growerid, 
    latitude, 
    longitude, 
   ...
   if ($conn->query($insert_sql)===TRUE) {
    ...
    // $last_id = $conn->insert_id;
     ...
     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','curing leaf quality','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
