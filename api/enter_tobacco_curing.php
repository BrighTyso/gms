...
$loading_density   = $_GET['loading_density'];
$leaf_arrangement  = $_GET['leaf_arrangement'];
$airflow           = $_GET['airflow'];
$curing_uniformity = $_GET['curing_uniformity'];
$tiers_condition   = $_GET['tiers_condition'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO tobacco_curing (
    userid, 
    growerid, 
    latitude, 
    longitude, 
   ...
   if ($conn->query($insert_sql)===TRUE) {
    ...
    // $last_id = $conn->insert_id;
     ...
     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','tobacco curing','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
