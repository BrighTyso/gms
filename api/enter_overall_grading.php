...
$packaging_suitability=$_GET['packaging_suitability'];
$market_readiness=$_GET['market_readiness'];
$estimated_price_impact=$_GET['estimated_price_impact'];
$overall_recommendations=$_GET['overall_recommendations'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO overall_grading(userid,
growerid,
latitude,
longitude,
created_at,
seasonid,
grading_status,
packaging_suitability,
market_readiness,
estimated_price_impact,
overall_recommendations,
datetimes) VALUES ($userid, $growerid, '$latitude', '$longitude', '$created_at', $seasonid, '$grading_status', '$packaging_suitability', '$market_readiness', '$estimated_price_impact', '$overall_recommendations', '$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','overall grading','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
