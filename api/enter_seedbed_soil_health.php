...
$seedbed_soil_drainage=validate($_GET['seedbed_soil_drainage']);
$seedbed_drainage_rate=validate($_GET['seedbed_drainage_rate']);
$seedbed_soil_structure=validate($_GET['seedbed_soil_structure']);
$seedbed_structure_rate=validate($_GET['seedbed_structure_rate']);
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO seedbed_soil_health(userid,  seasonid,  growerid,  latitude,  longitude,  created_at,  seedbed_soil_drainage,  seedbed_drainage_rate,  seedbed_soil_structure,  seedbed_structure_rate,datetimes) VALUES ($userid,$seasonid,$growerid, '$latitude',  '$longitude','$created_at','$seedbed_soil_drainage','$seedbed_drainage_rate','$seedbed_soil_structure','$seedbed_structure_rate','$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','seedbed soil health','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
