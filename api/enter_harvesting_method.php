...
$harvest_method=$_GET['harvest_method'];
 $leaf_type=$_GET['leaf_type'];
  $harvest_sequence=$_GET['harvest_sequence'];
  $handling_method=$_GET['handling_method'];
  $damage_observed=$_GET['damage_observed'];
  $leaves_per_harvest=$_GET['leaves_per_harvest'];
  $harvest_interval=$_GET['harvest_interval'];
  $leaf_loss_est=$_GET['leaf_loss_est'];

// userid, growerid, latitude, longitude, created_at, datetimes,seasonid, harvest_method, leaf_type, harvest_sequence,handling_method,damage_observed, leaves_per_harvest,harvest_interval,leaf_loss_est
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO harvesting_method(userid, growerid, latitude, longitude, created_at, datetimes,seasonid, harvest_method, leaf_type, harvest_sequence,handling_method,damage_observed, leaves_per_harvest,harvest_interval,leaf_loss_est) VALUES ($userid, 
$growerid, '$latitude','$longitude','$created_at', '$datetimes',$seasonid, '$harvest_method', '$leaf_type', '$harvest_sequence','$handling_method','$damage_observed', '$leaves_per_harvest','$harvest_interval','$leaf_loss_est')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','harvesting method','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
