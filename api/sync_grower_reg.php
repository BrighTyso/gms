...
if ($response==0) {
   ...
   if ($conn->query($grower_sql)===TRUE) {
	...
	$lat_long_sql = "INSERT INTO reg_location(userid,growerid,latitude,longitude,seasonid,hectares,created_at) VALUES ($userid,$last_id,'$lat','$long',$seasonid,'$hectares','$created_at')";
	   //$sql = "select * from login";
     ...
     $response111=0;
		...
		$sql = "Select id from reg_location where growerid=$growerid and seasonid=$seasonid";
		$result = $conn->query($sql);
   ...
   if ($response111==0) {
	...
	$lat_long_sql = "INSERT INTO reg_location(userid,growerid,latitude,longitude,seasonid,hectares,created_at) VALUES ($userid,$growerid,'$lat','$long',$seasonid,'$hectares','$created_at')";
	   //$sql = "select * from login";
	   if ($conn->query($lat_long_sql)===TRUE) {
