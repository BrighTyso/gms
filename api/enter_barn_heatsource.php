...
$flue_intact=$_GET['flue_intact'];
$smoke_leakages=$_GET['smoke_leakages'];
$temperature_control=$_GET['temperature_control'];
$thermometer=$_GET['thermometer'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO barn_heatsource(userid, 
growerid, 
latitude, 
longitude, 
created_at, 
...
'$thermometer')";
   ...
   if ($conn->query($insert_sql)===TRUE) {
    ...
    // $last_id = $conn->insert_id;
     ...
     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','barn heatsource','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
