...
$seedbed_type=validate($_GET['seedbed_type']);
$pocket_numbering=validate($_GET['pocket_numbering']);
$weeding_done=validate($_GET['weeding_done']);
$weeds_rate=validate($_GET['weeds_rate']);




$fertiliser_top=validate($_GET['fertiliser_top']);
$fertiliser_top_date=validate($_GET['fertiliser_top_date']);
$fungi_app=validate($_GET['fungi_app']);
$fungi_app_date=validate($_GET['fungi_app_date']);



$pesti_app=validate($_GET['pesti_app']);
$pesti_app_date=validate($_GET['pesti_app_date']);
$herbi_app=validate($_GET['herbi_app']);
$herbi_app_date=validate($_GET['herbi_app_date']);
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO seedbed_management(userid,   seasonid,  growerid,  latitude,  longitude,  created_at,  seedbed_type,  pocket_numbering,  weeding_done,  weeds_rate,  fertiliser_top,  fertiliser_top_date,  fungi_app,  fungi_app_date,  pesti_app,  pesti_app_date,  
herbi_app,  herbi_app_date,datetimes) VALUES ($userid,$seasonid,$growerid,'$latitude','$longitude','$created_at','$seedbed_type','$pocket_numbering','$weeding_done','$weeds_rate','$fertiliser_top','$fertiliser_top_date','$fungi_app','$fungi_app_date','$pesti_app',  '$pesti_app_date',  
'$herbi_app',  '$herbi_app_date','$datetimes')";
