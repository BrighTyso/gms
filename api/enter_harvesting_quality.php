...
$bruising_risk=$_GET['bruising_risk'];
 $overheating=$_GET['overheating'];
  $uneven_curing=$_GET['uneven_curing'];
$expected_grade=$_GET['expected_grade'];

// userid, growerid, latitude, longitude, created_at, datetimes,seasonid, bruising_risk, overheating, uneven_curing,
// expected_grade
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO harvesting_quality(userid, growerid, latitude, longitude, created_at, datetimes,seasonid, bruising_risk, overheating, uneven_curing,
expected_grade) VALUES ($userid, 
$growerid, '$latitude','$longitude','$created_at', '$datetimes',$seasonid, '$bruising_risk', '$overheating', '$uneven_curing',
'$expected_grade')";
   ...
   if ($conn->query($insert_sql)===TRUE) {
    ...
    // $last_id = $conn->insert_id;
     ...
     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','harvesting quality','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
