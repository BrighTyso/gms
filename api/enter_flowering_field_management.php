...
$weeding_done=$_GET['weeding_done'];
$weeds_level=$_GET['weeds_level'];
$herbicide=$_GET['herbicide'];
$harvest_ready=$_GET['harvest_ready'];
$ripening_percent=$_GET['ripening_percent'];
$expected_weight=$_GET['expected_weight'];
$labor_availability=$_GET['labor_availability'];
$uniformity_score=$_GET['uniformity_score'];
...
// then insert loan
  ...
  if ($growerid>0) {
   ...
   $insert_sql = "INSERT INTO flowering_field_management(userid,  growerid,   latitude,  longitude,  created_at,  seasonid,  weeding_done,  weeds_level,  herbicide, 
harvest_ready, ripening_percent, expected_weight, labor_availability, uniformity_score, datetimes) VALUES ($userid,  $growerid,  '$latitude',  '$longitude',  '$created_at',  $seasonid,  '$weeding_done',  '$weeds_level',  '$herbicide', 
'$harvest_ready', '$ripening_percent', '$expected_weight', '$labor_availability', '$uniformity_score', '$datetimes')";
   //$gr = "select * from login";
   if ($conn->query($insert_sql)===TRUE) {
   
    // $last_id = $conn->insert_id;

     $insert_sql = "insert into visits(userid,growerid,seasonid,latitude,longitude,created_at,description,times) value($userid,$growerid,$seasonid,'$latitude','$longitude','$created_at','flowering field management','$datetimes');";
       //$gr = "select * from login";
       if ($conn->query($insert_sql)===TRUE) {
